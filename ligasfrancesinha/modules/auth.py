...
from ligasfrancesinha.models import User
from ligasfrancesinha.tools import email_tools, auth_tools
...
def register():
    if request.method == 'POST':
        ...
        if not username:
            error = 'You need to add a username'
        elif not password:
            error = 'You need to add a password'
        elif User.query.filter_by(username=username).first() is not None:
            error = f"That username already exists on the database"
        elif User.query.filter_by(email=email).first() is not None:
            error = f"That email already exists on the database"
